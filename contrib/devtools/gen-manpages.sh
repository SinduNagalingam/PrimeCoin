...
LITECOIND=${LITECOIND:-$SRCDIR/primetalentcoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/primetalentcoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/primetalentcoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/primetalentcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
PTCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
