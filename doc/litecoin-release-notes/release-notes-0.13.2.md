primetalentcoin Core version 0.13.2 is now available from:

  <https://download.primetalentcoin.org/primetalentcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/primetalentcoin-project/primetalentcoin/issues>
...
updates will be released anymore. Without security updates, using a primetalentcoin
...
ECDSA signatures inside primetalentcoin transactions now use validation using
...
Previous versions of primetalentcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of primetalentcoin Core was to raise the default minimum
relay fee.

primetalentcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
primetalentcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running primetalentcoind without having to do any manual configuration.
...
primetalentcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. primetalentcoin Core will relay transactions with
...
entirely in primetalentcoin Core 0.13, and it is at this time undecided whether the
...
primetalentcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
primetalentcoin Core automatically creates a hidden service to listen on, without
manual configuration. primetalentcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if primetalentcoin Core is listening, and
...
primetalentcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals primetalentcoin
Core to use floating fees. By default, primetalentcoin Core will use floating
fees.
...
(default: `0.0002` PTC/kB).

At all times, primetalentcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PTC.
Furthermore, primetalentcoin Core will never create transactions paying less than
...
`primetalentcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /primetalentcoinrpc>
        ...
        # optional bypass primetalentcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< primetalentcoinrpc:<password>
    </Location>
...
Restarting primetalentcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in primetalentcoin.conf
...
primetalentcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/primetalentcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The primetalentcoin Core code base has
...
- `primetalentcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `primetalentcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the primetalentcoin scripting system that in combination with
...
primetalentcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of primetalentcoin Core.
...
The two phases are distinct in the primetalentcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming primetalentcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for primetalentcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each primetalentcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of primetalentcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of primetalentcoins being spent is exactly
...
- **Increased security for multisig:** primetalentcoin addresses (both P2PKH addresses
  ...
  believed to be provided by primetalentcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of primetalentcoin's current ledger (called the UTXO set),
  ...
  note that primetalentcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  primetalentcoin users to individually opt-in to almost any change in the primetalentcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and primetalentcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the primetalentcoin
client to primetalentcoin Core 0.13.2 to signal for segwit support. When a
...
Since primetalentcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `primetalentcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `primetalentcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
